...
class api_operations:
    ...
    def delete(db, model_name, model_param, param, name_in_mes):
        ...
        if delete_db["message"] == "Đã xóa":
            return {"success": True, "message": message_success}

        return {"success": False, "message": message_fail}
...
def get_phieunhaphang_by_madaily_and_date(
    madaily: int, start_date: date, end_date: date, db: Session = Depends(get_db)
):
    get_all_phieunhaphang = crud.get_phieunhaphang_by_madaily_and_date(
        db, madaily, start_date, end_date
    )
    result = []
    if get_all_phieunhaphang:
        for item in get_all_phieunhaphang:
        ...
        return result
    return {
        "message": "Không tìm thấy phiếu nhập hàng của đại lý trong khoảng thời gian này"
    }
...
def add_new_phieunhaphang(
    ...
    try:
        ...
        # Update sotienno in Daily table
        daily = db.query(models.Daily).filter_by(madaily=madaily).first()
        if daily:
            daily.sotienno = total_amount
...
@app.put("/phieunhaphang/capnhat/maphieunhap/{maphieunhap}/{madaily}")
...
class DeleteImportBill(BaseModel):
    ...
    debt: int  # Store's debt remain


@app.delete("/phieunhaphang/xoa/maphieunhap/{maphieunhap}/{madaily}")
def delete_phieunhaphang(
    ...
    try:
        ...
        if phieunhaphang and daily:
            ...
            daily.sotienno -= delete_import_bill.debt
...
def get_phieuxuathang_by_madaily_and_date(
    madaily: int, start_date: date, end_date: date, db: Session = Depends(get_db)
):
    get_all_phieuxuathang = crud.get_phieuxuathang_by_madaily_and_date(
        db, madaily, start_date, end_date
    )
    result = []
    if get_all_phieuxuathang:
        ...
        return result
    return {
        "message": "Không tìm thấy phiếu xuất hàng của đại lý trong khoảng thời gian này"
    }
