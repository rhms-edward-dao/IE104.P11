# coding: utf-8
from sqlalchemy import Boolean, CheckConstraint, Column, Date, DateTime, ForeignKey, Index, Integer, Numeric, SmallInteger, String, Table, Text, text
...
class Chucvu(Base):
    ...
    machucvu = Column(SmallInteger, primary_key=True, server_default=text("nextval('chucvu_machucvu_seq'::regclass)"))
...
class Khachhang(Base):
    ...
    makhachhang = Column(SmallInteger, primary_key=True, server_default=text("nextval('khachhang_makhachhang_seq'::regclass)"))
...
class Loaidaily(Base):
    ...
    maloaidaily = Column(SmallInteger, primary_key=True, server_default=text("nextval('loaidaily_maloaidaily_seq'::regclass)"))
...
class Loaimathang(Base):
    ...
    maloaimathang = Column(SmallInteger, primary_key=True, server_default=text("nextval('loaimathang_maloaimathang_seq'::regclass)"))
...
class Quan(Base):
    ...
    maquan = Column(SmallInteger, primary_key=True, server_default=text("nextval('quan_maquan_seq'::regclass)"))
...
class Taikhoan(Base):
    ...
    mataikhoan = Column(SmallInteger, primary_key=True, server_default=text("nextval('taikhoan_mataikhoan_seq'::regclass)"))
...
class Daily(Base):
    ...
    madaily = Column(SmallInteger, primary_key=True, server_default=text("nextval('daily_madaily_seq'::regclass)"))
    maloaidaily = Column(ForeignKey('loaidaily.maloaidaily', ondelete='SET NULL'), nullable=False)
...
class KhachhangDiachi(Base):
    ...
    makhachhang = Column(ForeignKey('khachhang.makhachhang', ondelete='CASCADE'), primary_key=True, nullable=False)
...
class Baocaocongno(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("date_part('day'::text, ngaylapbaocao) < (29)::double precision"),
    ...
    madaily = Column(ForeignKey('daily.madaily', ondelete='CASCADE'), primary_key=True, nullable=False)
...
class Baotridaily(Base):
    ...
    mabaotri = Column(SmallInteger, primary_key=True, server_default=text("nextval('baotridaily_mabaotri_seq'::regclass)"))
    madaily = Column(ForeignKey('daily.madaily', ondelete='CASCADE'), nullable=False)
    thoidiembatdau = Column(DateTime, index=True, server_default=text("CURRENT_TIMESTAMP"))
...
class DailyDiachi(Base):
    ...
    madaily = Column(ForeignKey('daily.madaily', ondelete='CASCADE'), primary_key=True, nullable=False)
    maquan = Column(ForeignKey('quan.maquan', ondelete='CASCADE'), primary_key=True, nullable=False)
...
class Mathang(Base):
    ...
    mamathang = Column(SmallInteger, primary_key=True, server_default=text("nextval('mathang_mamathang_seq'::regclass)"))
...
class Nhanvien(Base):
    ...
    manhanvien = Column(SmallInteger, primary_key=True, server_default=text("nextval('nhanvien_manhanvien_seq'::regclass)"))
...
class Phieunhaphang(Base):
    ...
    __table_args__ = (
        Index('idx_pnh_madaily_ngaylapphieu', 'madaily', 'ngaylapphieu'),
    )

    maphieunhap = Column(SmallInteger, primary_key=True, server_default=text("nextval('phieunhaphang_maphieunhap_seq'::regclass)"))
    madaily = Column(ForeignKey('daily.madaily', ondelete='CASCADE'), nullable=False, index=True)
    ...
    trinhtrang = Column(String(50), server_default=text("'Còn nợ'::character varying"))
...
class Phieuxuathang(Base):
    ...
    __table_args__ = (
        Index('idx_pxh_madaily_ngaylapphieu', 'madaily', 'ngaylapphieu'),
    )

    maphieuxuat = Column(SmallInteger, primary_key=True, server_default=text("nextval('phieuxuathang_maphieuxuat_seq'::regclass)"))
    ngaylapphieu = Column(Date, index=True, server_default=text("CURRENT_DATE"))
    tongtien = Column(Numeric(12, 4), nullable=False)
    madaily = Column(ForeignKey('daily.madaily', ondelete='CASCADE'), nullable=False, index=True)
    makhachhang = Column(ForeignKey('khachhang.makhachhang', ondelete='CASCADE'), nullable=False)
...
class ChitietPnh(Base):
    ...
    mact_pnh = Column(Integer, primary_key=True, server_default=text("nextval('chitiet_pnh_mact_pnh_seq'::regclass)"))
    maphieunhap = Column(ForeignKey('phieunhaphang.maphieunhap', ondelete='CASCADE'), nullable=False, index=True)
    mamathang = Column(ForeignKey('mathang.mamathang', ondelete='SET NULL'), nullable=False, index=True)
...
class ChitietPxh(Base):
    ...
    mact_pxh = Column(Integer, primary_key=True, server_default=text("nextval('chitiet_pxh_mact_pxh_seq'::regclass)"))
    maphieuxuat = Column(ForeignKey('phieuxuathang.maphieuxuat', ondelete='CASCADE'), nullable=False, index=True)
    mamathang = Column(ForeignKey('mathang.mamathang', ondelete='SET NULL'), nullable=False, index=True)
...
class NhanvienChucvu(Base):
    ...
    __table_args__ = (
        CheckConstraint('thoihan >= (0)::numeric'),
    )

    manhanvien = Column(ForeignKey('nhanvien.manhanvien', ondelete='CASCADE'), primary_key=True, nullable=False)
    machucvu = Column(ForeignKey('chucvu.machucvu'), primary_key=True, nullable=False)
    ngaybatdau = Column(Date, primary_key=True, nullable=False, server_default=text("CURRENT_DATE"))
...
t_taikhoan_nhavien = Table(
    'taikhoan_nhavien', metadata,
    Column('manhanvien', ForeignKey('nhanvien.manhanvien', ondelete='CASCADE'), primary_key=True, nullable=False),
    Column('mataikhoan', ForeignKey('taikhoan.mataikhoan', ondelete='CASCADE'), primary_key=True, nullable=False)
)
