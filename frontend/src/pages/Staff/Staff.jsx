...
import { getAllStaff, deleteStaff, getAllPosition } from '../../assets/StaffData';
...
function Staff() {
        ...
        if (staffSearchTerm.trim() !== '') {
            const results = staffData.filter(item => {
                if (selectedOption === "Tên nhân viên") {
                    return item.Nhanvien.hoten.toLowerCase().includes(staffSearchTerm.toLowerCase())
                } else if (selectedOption === "Đại lý") {
                    return item.tendaily.toLowerCase().includes(staffSearchTerm.toLowerCase())
                } else {
                    return item.tenchucvu.toLowerCase().includes(staffSearchTerm.toLowerCase())
                }
            });
        ...
        if (positionSearchTerm.trim() !== '') {
            const presult = positionData.filter(item => {
                if (selectedOptionPos === "Tên chức vụ") {
                    return item.tenchucvu.toLowerCase().includes(positionSearchTerm.toLowerCase())
                } else if (selectedOptionPos === "Cấp độ") {
                    return item.capdo.toString().includes(positionSearchTerm.toLowerCase())
                } else {
                    return item.thoihan.toString().includes(positionSearchTerm.toLowerCase())
                }
            });
    ...
    const position_items = positionSearchTerm ? positionSearchResults : positionData;
    // Offset pagination
    const offsetStaff = currentPageStaff * itemsPerPage;
    const currentStaffItems = staff_items.length > 0 ? staff_items.slice(offsetStaff, offsetStaff + itemsPerPage) : [];
    const pageCountStaff = staff_items.length > 0 ? Math.ceil(parseFloat(staff_items.length / itemsPerPage)) : 0;
        ...
        return (
            <div>
                ...
                { DWCardData.map((card, index) => (
                    <Card key={index} image={card.img} description={card.description} value={card.value}></Card>
                ))}                  
            </div>
                    ...
                    <table className='w-full mt-5 text-center'>
                        <thead className='border-b-4 border-red-500'>
                            <tr className='text-md text-gray-500'>
                                ...
                                <th scope='col' className='py-5 border-r-2'>Tên nhân viên</th>
                                <th scope='col' className='py-5 border-r-2'>Ngày sinh</th>                                
                                <th scope='col' className='py-5 border-r-2'>Tên đại lý</th>
                                <th scope='col' className='py-5 border-r-2'>Chức vụ</th>
                                ...
                                <>
                                    {staff_items.map((list, index) => (
                                        <tr key={index} className='border-b border-slate-300 hover:bg-slate-200'> 
                                            ...
                                            <td scope="row" className='py-5 border-r-2'>{list.Nhanvien.hoten}</td>                                        
                                            <td scope='row' className='py-5 border-r-2'>{list.Nhanvien.ngaysinh}</td>
                                            ...
                                            <td scope="row">                                
                                                <div className='flex flex-wrap m-2 gap-3'>
                                                    ...
                                                    <button onClick={() => deleteData(list.Nhanvien.manhanvien)}>
                                                        <div className='flex gap-2 bg-amber-400 p-2 rounded-md'>
                                                            <img src={TrashIcon} alt="Icon thùng rác" />
                                                            <p className='font-bold text-white hidden lg:block'>Xóa</p>
                                                        </div>
                                                    </button>
                                                </div>
                                            </td>
                ...
                <div className='mt-5'>                
                    ...
                    <table className='w-full mt-5 text-center'>
                        <thead className='border-b-4 border-red-500'>
                            <tr className='text-md text-gray-500'>
                                <th scope='col'></th>
                                <th scope='col' className='py-5 border-r-2'>Tên chức vụ</th>
                                <th scope='col' className='py-5 border-r-2'>Cấp độ</th>
                                <th scope='col' className='py-5 border-r-2'>Lương</th>
                                <th scope='col' className='py-5 border-r-2'>Thời hạn</th>
                                <th scope='col' className='py-5 border-r-2'>Ngày tạo</th>
                                <th scope='col' className='py-5 border-r-2'>Ngày cập nhật</th>
                                <th scope='col'></th>
                            </tr>
                        </thead>
                        <tbody>
                                ...
                                <>
                                    {position_items.map((list, index) => (
                                        <tr key={index} className='border-b border-slate-300 hover:bg-slate-200'> 
                                            ...
                                            <td scope="row" className='py-5 border-r-2'>{list.tenchucvu}</td>
                                            <td scope="row" className='py-5 border-r-2'>{list.capdo}</td>
                                            <td scope="row" className='py-5 border-r-2'>{list.luong}</td>
                                            <td scope="row" className='py-5 border-r-2'>{list.thoihan}</td>
                                            ...
                                            <td scope="row">                                
                                                <div className='flex flex-wrap justify-between m-2 gap-3'>
                                                    ...
                                                    <button onClick={() => deletePositionData(list.machucvu)}>
                                                        <div className='flex gap-2 bg-amber-400 p-2 rounded-md'>
                                                            <img src={TrashIcon} alt="Icon thùng rác" />
