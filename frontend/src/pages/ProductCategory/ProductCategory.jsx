...
function ProductCategorys() {
  ...
  const [selectedOptionCat, setSelectedOptionCat] = useState("Tên loại mặt hàng");
    ...
    if (productSearchTerm.trim() !== "") {
      const presult = productData.filter((item) => {
        if (selectedOption === "Tên mặt hàng") {
          return item.Mathang.tenmathang.toLowerCase().includes(productSearchTerm.toLowerCase())
        } else {
          return item.tenloaimathang.toLowerCase().includes(productSearchTerm.toLowerCase())
        }
      });
  ...
  }, [searchTerm, productSearchTerm, data, productData, selectedOption, selectedOptionCat]);
          ...
          <table className="w-full mt-5 text-center">
            <thead className="border-b-4 border-red-500">
              <tr className="text-md text-gray-500">
                <th scope="col"></th>
                <th scop="col" className="py-5 border-r-2">Hình ảnh</th>
