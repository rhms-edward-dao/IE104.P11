...
export const getStaffById = async (id) => {
  try {
    const response = await fetch(
      `http://127.0.0.1:8000/nhanvien/manhanvien/${id}`
    );
    const data = await response.json();
    return data;
  } catch (error) {
...
export const getStaffDetail = async (id) => {
  try {
    const response = await fetch(
      `http://127.0.0.1:8000/nhanvien/chitiet/${id}`
    );
    const data = await response.json();
    return data;
  } catch (error) {
...
export const updateStaff = async (id, item) => {
  try {
    ...
    // Send data to server
    const response = await fetch(
      `http://127.0.0.1:8000/nhanvien/capnhat/${id}`,
      {
        method: "PUT",
        body: formData,
      }
...
export const updatePosition = async (machucvu, item) => {
  try {
    ...
    formData.append("thoihan", item.thoihan);
    const response = await fetch(
      `http://127.0.0.1:8000/chucvu/capnhat/${machucvu}`,
      {
        method: "PUT",
        body: formData,
      }
