...
export const getAllCategoryName = async() => {
    try {
        const response = await fetch(`http://127.0.0.1:8000/loaimathang/tatca/tenloaimathang`);
        const data = await response.json();
        return data;
    }
    catch (error) {
...
export const getProductTypeById = async(id) => {
    try {
        const response = await fetch(`http://127.0.0.1:8000/loaimathang/maloaimathang/${id}`)
        const data = await response.json()
        return data;
    }
    catch (error) {
...
export const updateOneCategory = async (id, tenloaimathang) => {
    try {
        const response = await fetch(`http://127.0.0.1:8000/loaimathang/capnhat/${id}`, {
            method: 'PUT',
            headers: {
                "Content-Type": "application/json"
            },
...
export const deleteOneCategory = async (id) => {
    try {
        const response = await fetch(`http://127.0.0.1:8000/loaimathang/xoa/${id}`, { method: 'DELETE' });
        if (!response.ok) {
